...
func (br *budgetRepository) FindYearAndSource(c context.Context, id string) (*sql.Row, error) {
	query := "SELECT budgets.id, budgets.price, years.year, sources.name, budgets.created_at, budgets.updated_at FROM budgets INNER JOIN years ON budgets.year_id = years.id INNER JOIN sources ON budgets.source_id = sources.id where budgets.id = " + id
	row := br.client.DB().QueryRowContext(c, query)
	fmt.Printf("\x1b[36m%s\n", query)
	return row, nil
}
