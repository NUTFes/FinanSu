...
func (p *purchaseOrderController) CreatePurchaseOrder(c echo.Context) error{
	...
	purchaseOrder,err := p.u.CreatePurchaseOrder(c.Request().Context(), deadLine, userID, financeCheck)
	if err != nil {
		return err
	}
	return c.JSON(http.StatusOK, purchaseOrder)
...
func (p *purchaseOrderController) UpdatePurchaseOrder(c echo.Context) error{
	...
	purchaseOrder,err := p.u.UpdatePurchaseOrder(c.Request().Context(), id, deadLine, userID, financeCheck)
	if err != nil {
		return err
	}
	return c.JSON(http.StatusOK, purchaseOrder)
...
func (p *purchaseOrderController) IndexOrderWithUserItem(c echo.Context) error{
	orderWithUserItems, err := p.u.GetPurchaseOrderDetails(c.Request().Context())
	if err != nil {
		return err
	}
...
func (p *purchaseOrderController) ShowOrderWithUserItem(c echo.Context) error{
	id := c.Param("id")
	orderWithUserAndItem , err := p.u.GetPurchaseOrderDetailByID(c.Request().Context(),id)
	if err != nil {
		return err
	}
