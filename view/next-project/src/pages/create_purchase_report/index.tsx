...
const FORM_CONTAINER_CLASS =
	"w-full min-w-[300px] max-w-[60%] px-4 py-8 sm:px-6 lg:px-8";
...
const PurchaseReportPage = () => {
	...
	if (isEditMode && isReportDataLoading) {
		return (
			<MainLayout>
				<Box
					className={`flex ${CONTAINER_HEIGHT_CLASS} items-center justify-center font-noto`}
				>
					<Center>
						<Spinner size="xl" />
					...
					<form className="space-y-6">
						<VStack spacing={4} align="stretch">
								...
								{isEditMode && festivalItemName ? (
									...
									>
										{filteredFestivalItems.map((item) => (
											<option
												key={item.festivalItemId}
												value={item.festivalItemId}
											>
												{item.name}
											</option>
										))}
							...
							<FormControl
								id="amount"
								isRequired
								isInvalid={!!formErrors.amountError}
							>
