...
export default function PurchaseReports() {
	...
	useEffect(() => {
		if (buyReports.length > 0) {
			setSealChecks(
				Object.fromEntries(
					buyReports.map((report) => [report.id, report.isPacked ?? false]),
				),
			);
			setSettlementChecks(
				Object.fromEntries(
					buyReports.map((report) => [report.id, report.isSettled ?? false]),
				),
			);
		}
	}, [buyReports]);
	...
	const { trigger, error: statusError } =
		usePutBuyReportStatusBuyReportId(buyReportId);
	...
	// エラーハンドリングもuseEffect内に移動
	useEffect(() => {
		if (yearPeriodsError || buyReportsError) {
			router.push("/500");
		}
	}, [yearPeriodsError, buyReportsError, router]);
	...
	if (yearPeriodsError || buyReportsError) return null;
							...
							<table className="mb-5 table-auto border-collapse">
								...
								<tbody>
									{buyReports && buyReports.length > 0 ? (
												...
												<td>
													<div className="flex">
														...
														<div className="mx-1">
															<EditButton
																isDisabled={
																	sealChecks[report.id ?? 0] &&
																	settlementChecks[report.id ?? 0]
																}
										...
										<tr>
											<td
												className="border-b border-primary-1 px-1 py-3"
												colSpan={9}
											>
