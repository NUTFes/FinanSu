import clsx from 'clsx';
import { useRouter } from 'next/router';
import React, { useState } from 'react';
import { RiExternalLinkLine, RiFileCopyLine } from 'react-icons/ri';
import { RiArrowDropRightLine } from 'react-icons/ri';

import { post } from '@api/sponsorActivitiesItem';
import {
  PrimaryButton,
  OutlinePrimaryButton,
  CloseButton,
  Input,
  Modal,
  Stepper,
  Tooltip,
} from '@components/common';
import { SponsorActivitiesItem } from '@type/common';

interface ModalProps {
  sponsorActivitesItemNum: SponsorActivitiesItemNum;
  isOpen: boolean;
  numModalOnClose: () => void;
  onClose: () => void;
  setFormDataList: (formDataList: SponsorActivitiesItem[]) => void;
  formDataList: SponsorActivitiesItem[];
}

interface SponsorActivitiesItemNum {
  value: number;
}

export default function AddModal(props: ModalProps) {
  const [activeStep, setActiveStep] = useState<number>(1);
  const nextStep = () => {
    setActiveStep(activeStep + 1);
  };
  const prevStep = () => {
    setActiveStep(activeStep - 1);
  };
  const reset = () => {
    setActiveStep(1);
    setIsDone(false);
  };

  const [isDone, setIsDone] = useState<boolean>(false);
  const router = useRouter();

  const handler =
    (stepNumber: number, input: string) =>
    (e: React.ChangeEvent<HTMLSelectElement> | React.ChangeEvent<HTMLInputElement>) => {
      props.setFormDataList(
        props.formDataList.map((formData: SponsorActivitiesItem) =>
          formData.id === stepNumber ? { ...formData, [input]: e.target.value } : formData,
        ),
      );
    };

  const addSponsorActivitiesItem = async (data: SponsorActivitiesItem[]) => {
    const addPurchaseItemUrl = process.env.CSR_API_URI + '/activities';
    data.map(async (item) => {
      await post(addPurchaseItemUrl, item);
    });
  };

  const submit = async (formDataList: SponsorActivitiesItem[]) => {
    addSponsorActivitiesItem(formDataList);
    props.onClose();
    props.numModalOnClose();
    router.reload();
  };

  // 購入物品の情報
  const content = (index: number, data: SponsorActivitiesItem) => (
    <>
      <div className={clsx('my-6 grid grid-cols-12 gap-4')}>
        <div className={clsx('col-span-2 mr-2 grid')}>
          <div className={clsx('text-md flex grid items-center justify-items-end text-black-600')}>
            協賛ID
          </div>
        </div>
        <div className={clsx('col-span-10 grid w-full')}>
          <Input value={data.sponsorID} onChange={handler(index, 'sponsorID')} />
        </div>
        <div className={clsx('col-span-2 mr-2 grid')}>
          <div className={clsx('text-md flex grid items-center justify-items-end text-black-600')}>
            協賛スタイルID
          </div>
        </div>
        <div className={clsx('col-span-10 grid w-full')}>
          <Input value={data.sponsorStyleID} onChange={handler(index, 'sponsorStyleID')} />
        </div>
        <div className={clsx('col-span-2 mr-2 grid')}>
          <div className={clsx('text-md flex grid items-center justify-items-end text-black-600')}>
            ユーザID
          </div>
        </div>
        <div className={clsx('col-span-10 grid w-full')}>
          <Input value={data.userID} onChange={handler(index, 'userID')} />
        </div>
      </div>
    </>
  );

  // 購入物品テーブルのカラム
  const tableColumns = ['協賛ID', '協賛スタイルID', 'ユーザID'];

  // 購入物品の確認用テーブル
  const SponsorActivitiesItemTable = (sponsorActivitesItems: SponsorActivitiesItem[]) => {
    return (
      <table className={clsx('table-fixed border-collapse')}>
        <thead>
          <tr className={clsx('border border-x-white-0 border-b-primary-1 border-t-white-0 py-3')}>
            {tableColumns.map((tableColumn: string) => (
              <th key={tableColumn} className={clsx('border-b-primary-1 px-6 pb-2')}>
                <div className={clsx('text-center text-sm text-black-600')}>{tableColumn}</div>
              </th>
            ))}
          </tr>
        </thead>
        <tbody className={clsx('border border-x-white-0 border-b-primary-1 border-t-white-0')}>
          {sponsorActivitesItems.map((sponsorActivitesItem, index) => (
            <tr key={sponsorActivitesItem.id}>
              <td
                className={clsx(
                  'px-4',
                  index === 0 ? 'pt-4 pb-3' : 'py-3',
                  index === sponsorActivitesItems.length - 1 ? 'pb-4 pt-3' : 'border-b py-3',
                )}
              >
                <div className={clsx('text-center text-sm text-black-300')}>
                  {sponsorActivitesItem.sponsorID}
                </div>
              </td>
              <td
                className={clsx(
                  'px-4',
                  index === 0 ? 'pt-4 pb-3' : 'py-3',
                  index === sponsorActivitesItems.length - 1 ? 'pb-4 pt-3' : 'border-b py-3',
                )}
              >
                <div className={clsx('text-center text-sm text-black-300')}>
                  {sponsorActivitesItem.sponsorStyleID}
                </div>
              </td>
              <td
                className={clsx(
                  'px-4',
                  index === 0 ? 'pt-4 pb-3' : 'py-3',
                  index === sponsorActivitesItems.length - 1 ? 'pb-4 pt-3' : 'border-b py-3',
                )}
              >
                <div className={clsx('text-center text-sm text-black-300')}>
                  {sponsorActivitesItem.userID}
                </div>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    );
  };

  // 購入物品数だけステップを用意
  const steps = [];
  for (let i = 0; i < props.sponsorActivitesItemNum.value; i++) {
    steps.push({ label: '' });
  }

  return (
    <>
      <Modal className='!w-1/2'>
        <div className={clsx('w-full')}>
          <div className={clsx('mr-5 grid w-full justify-items-end')}>
            <CloseButton
              onClick={() => {
                props.onClose();
                props.numModalOnClose();
              }}
            />
          </div>
        </div>
        <div className={clsx('mb-10 grid w-full justify-items-center text-xl text-black-600')}>
          協賛企業の登録
        </div>
        <div className={clsx('my-6 grid grid-cols-12 gap-4')}>
          <div className={clsx('col-span-1 grid')} />
          <div className={clsx('col-span-10 grid w-full')}>
            <Stepper stepNum={props.sponsorActivitesItemNum.value} activeStep={activeStep} isDone={isDone}>
              {!isDone && <>{content(activeStep, props.formDataList[activeStep - 1])}</>}
            </Stepper>
            {isDone ? (
              <>
                <div
                  className={clsx(
                    'text-md h-100 grid w-full justify-items-center font-bold text-black-300',
                  )}
                >
                  購入物品
                </div>
                <div className={clsx('mb-2 grid w-full justify-items-center p-5')}>
                  {SponsorActivitiesItemTable(props.formDataList)}
                </div>
                <div className={clsx('my-10 grid grid-cols-12 gap-4')}>
                  <div className={clsx('col-span-1 grid')} />
                  <div className={clsx('col-span-10 grid w-full justify-items-center')}>
                    <div className={clsx('flex')}>
                      <OutlinePrimaryButton onClick={reset} className={'mx-2'}>
                        戻る
                      </OutlinePrimaryButton>
                      <PrimaryButton
                        className={'mx-2'}
                        onClick={() => {
                          submit(props.formDataList);
                        }}
                      >
                        登録
                      </PrimaryButton>
                    </div>
                  </div>
                </div>
              </>
            ) : (
              <>
                <div className={clsx('mt-6 grid grid-cols-12 gap-4')}>
                  <div className={clsx('col-span-1 grid')} />
                  <div className={clsx('col-span-10 grid justify-items-center')}>
                    <div className={clsx('flex')}>
                      {/* stepが1より大きい時のみ戻るボタンを表示 */}
                      {activeStep > 1 && (
                        <OutlinePrimaryButton onClick={prevStep} className={'mx-2'}>
                          戻る
                        </OutlinePrimaryButton>
                      )}
                      <PrimaryButton
                        className={'mx-2 pl-4 pr-2'}
                        onClick={() => {
                          {
                            activeStep === steps.length ? setIsDone(true) : nextStep();
                          }
                        }}
                      >
                        <div className={clsx('flex')}>
                          {activeStep === steps.length ? '申請の確認' : '次へ'}
                          <RiArrowDropRightLine size={23} />
                        </div>
                      </PrimaryButton>
                    </div>
                  </div>
                  <div className={clsx('col-span-1 grid')} />
                </div>
              </>
            )}
          </div>
          <div className={clsx('col-span-1 grid')} />
        </div>
      </Modal>
    </>
  );
}
