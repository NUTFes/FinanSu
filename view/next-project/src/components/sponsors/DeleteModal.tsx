...
const SponsorDeleteModal: FC<ModalProps> = (props) => {
  ...
  const deleteSponsorStyle = async (id: number | string) => {
    const deletePurchaseReportUrl = process.env.CSR_API_URI + '/sponsors/' + id;
  ...
  return (
    <Modal className='md:w-1/2' onClick={() => props.setShowModal(false)}>
      ...
      <div className='text-black-600 mx-auto mb-5 w-fit text-xl'>企業情報の削除</div>
