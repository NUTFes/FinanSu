    ...
    <div className={clsx('h-full w-full bg-primary-4')}>
      <div className={clsx('border-b-2 border-primary-1')}>
        {FinanceLinkItems.map((link) => (
          <NavItem key={link.name} icon={link.icon} href={link.href} currentPath={router.pathname} isParent={link.isParent} isShow={financeItemsShow} onClick={link.isParent?setFinanceItemsShow:undefined}>
            {link.name}
          </NavItem>
        ))}
      </div>
      <div className={clsx('border-b-2 border-primary-1')}>
        {RelationLinkItems.map((link) => (
          <NavItem key={link.name} icon={link.icon} href={link.href} currentPath={router.pathname} isParent={link.isParent} isShow={relationItemsShow} onClick={link.isParent?setRelationItemsShow:undefined}>
            {link.name}
          </NavItem>
        ))}
      </div>
...
const NavItem = (props: NavItemProps) => {
  ...
  return (
    <>
      {props.isParent ? (
          <div className={clsx(className + ` cursor-pointer ${!props.isShow && 'border-b-2 border-dashed border-primary-1'}`)} onClick={ props.onClick }>
            <span className={clsx(props.isShow ? 'rotate-180 transition-transform' : 'rotate-0 transition-transform')}>{props.icon}</span>
            {props.children}
          </div>
      ) : (
        <a href={props.href}>
